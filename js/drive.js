...
export const Drive = {
  ...
  async init() {
    ...
    // Constructing the token client is local/offline — it never contacts
    // Google or shows anything to the user — so it's safe to do on every
    // app load regardless of page. Actually requesting a token (silent or
    // not) is the part that can surface a sign-in prompt, and that only
  ...
  // connected Drive backup but has no valid cached token right now (e.g.
  // the app was closed long enough for the ~1hr token to expire). This is
  // deliberately NOT called from init()/app startup — it's only called from
  // the Settings page (see views/settings.js), which is "the backup
  // section" — so the one kind of Google sign-in activity that can happen
  // without an explicit Connect click (a silent FedCM reconnect, which can
  // still flash a native browser account UI) only ever happens there, never
  // just from opening the app to Inbox/Clarify/etc.
  ...
  async signIn() {
    ...
    this._scheduleProactiveRefresh();
    this._wireVisibilityCheck();
    // Deliberately no backupNow() here: js/sync.js listens for the
    // signed-in transition and runs a real pull-merge-push, so a
    // freshly-connected empty device pulls existing Drive data in instead
    // of a raw backupNow() overwriting it with nothing.
  },
  ...
  // Uploads a JSON body to the single canonical backup file, creating it if
  // it doesn't exist yet. Shared by backupNow() (raw overwrite, used by the
  // manual "Back up now" button) and js/sync.js's merge-aware sync flow.
  async _uploadSnapshot(token, folderId, body) {
  ...
  // Downloads and parses the current canonical backup file, or null if none
  // exists in Drive yet. Shared by restoreLatest() and js/sync.js.
  async _downloadSnapshot(token, folderId) {
  ...
  async backupNow() {
    ...
    try {
      ...
      await this._uploadSnapshot(token, folderId, JSON.stringify(snapshot, null, 2));
  ...
  async restoreLatest() {
    const token = await this._ensureToken();
    const folderId = await this._findOrCreateFolder(token);
    const data = await this._downloadSnapshot(token, folderId);
    if (!data) throw new Error('No backup found in Drive yet.');
