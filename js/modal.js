...
import { el, escapeHtml, toast, uid } from './utils.js';
...
export async function openItemForm({ item = null, type, defaults = {}, onSaved, focusSection = false }) {
  ...
  const form = el('form', { class: 'form' }, [
    ...
    ['next-action', 'waiting-for', 'someday', 'calendar'].includes(type)
      ? field('Project (optional)', selectEl('projectId', projects.map((p) => p.title), projects.find((p) => p.id === data.projectId)?.title, projects))
      : null,
  ...
  form.addEventListener('submit', async (e) => {
    ...
    if (fd.has('projectId')) {
      const title = fd.get('projectId');
      const proj = projects.find((p) => p.title === title);
      record.projectId = proj ? proj.id : null;
    }
