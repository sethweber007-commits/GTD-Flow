...
export async function renderSettings() {
  ...
  function paintDriveUI(status) {
    statusRow.textContent = status.message + (status.lastBackupAt ? ` · Last backup ${formatDateTime(status.lastBackupAt)}` : '');
    btnRow.innerHTML = '';
    ...
    if (status.state === 'signed-in' || status.state === 'backing-up' || status.state === 'error') {
      btnRow.appendChild(el('button', { class: 'btn btn-primary', onclick: () => Drive.backupNow().then(() => toast('Backed up')).catch(() => toast('Backup failed', 'error')) }, 'Back up now'));
      btnRow.appendChild(
